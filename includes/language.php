<?php
  ...
  function i18n ( string $msgkey, ...$replaces ) {

    global $__i18n_msg;

    $key = trim( strtolower( $msgkey ) );
    $msg = array_key_exists( $key, $__i18n_msg ) ? $__i18n_msg[ $key ] : '›' . $msgkey . '‹';
  ...
  function i18n_save ( string $msgkey, ...$replaces ) {
    return strpos( $msg = i18n( $msgkey, ...$replaces ), '›' ) === 0 ? null : $msg;
  }

  function _i18n ( string $msgkey, ...$replaces ) {
    echo i18n( $msgkey, ...$replaces );
  }

  function i18n_load ( string $lang = '', bool $set_cookie = false ) {
    ...
    $__i18n_msg = json_decode( file_get_contents( LANG . $load_lang . '.json' ), true );
