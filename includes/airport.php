<?php
  ...
  function airport_count ( string $by = 'type' ) {
  ...
  function airport_by ( string $column, string $ident ) {
  ...
  function airport_image ( string $ICAO ) {
    ...
    return ( $res = $DB->query( '
      ...
      LIMIT   0, 1
    ' ) )->num_rows == 1 ? $res->fetch_assoc() : null;

  }

  function airport_wiki ( string $ICAO, string $lang = '' ) {

    global $DB;

    if ( empty( $lang ) ) $lang = i18n_locale();
  ...
  function airport_timezone ( array $airport ) {
  ...
  function airport_search ( string $word, int $limit = -1, int $offset = 0 ) {
  ...
  function airport_nearest ( float $lat, float $lon, array $options = [], int $max_deg = 10, int $limit = 99999 ) {

    global $DB;

    $query = '1';

    if ( count( $options ) ) $query = implode( ' AND ', array_filter( array_map( function ( $o ) {
  ...
  function airport_weather ( array $airport, int $max_deg = 10, int $max_age = 1 ) {
    ...
    ' ) )->num_rows > 0 ) {
      ...
      while ( $row = $res->fetch_assoc() ) {
        ...
        if ( empty( $first ) ) $first = $row;
        if ( $row->flight_cat != null ) return $row;
  ...
  function airport_warn ( array $airport ) {
  ...
  function _airport_warn ( array $airport ) {
    echo airport_warn( $airport );
  }

  function airport_list ( array $airports, int $page = 1, string $baseurl = '', array $point = [] ) {
    ...
    if ( count( $airports ) == 0 ) {
      ...
      foreach ( array_slice( $airports, ( $page - 1 ) * 24, 24 ) as $airport ) {
        ...
        $content .= '<div class="row airport-' . $airport[ 'type' ] . ' restriction-' . $airport[ 'restriction' ] . ' service-' . $airport[ 'service' ] . '">
          ...
          ' . ( !empty( $point ) ? __nearby( $airport[ 'distance' ], [ $airport[ 'lat' ], $airport[ 'lon' ] ], $point ) : '' ) . '
        </div>';
  ...
  function _airport_list ( array $airports, int $page = 1, string $baseurl = '', array $point = [] ) {
    echo airport_list( $airports, $page, $baseurl, $point );
  }

  function radio_list ( array $airport, array $radios ) {

    $list = '';

    foreach ( $radios as $radio ) $list .= '<div class="radio radio-' . $radio[ 'type' ] . '">
  ...
  function _radio_list ( array $airport, array $radios ) {
    echo radio_list( $airport, $radios );
  }

  function runway_list ( array $airport, array $runways ) {
  ...
  function _runway_list ( array $airport, array $runways ) {
    echo runway_list( $airport, $runways );
  }

  function alt_in ( float $altitude, string $in = 'ft' ) {
    return '<x>' . __number( $altitude ) . '</x>&#8239;' . $in;
  }

  function format_freq ( float $frequency ) {

    if ( $frequency > 1000 ) { $frequency /= 1000; $suffix = 'MHz'; }
    else $suffix = 'kHz';
  ...
  function __DMS ( float $decimal, string $type = 'lat' ) {
    ...
    return $deg . '°&#8239;' . $min . '′&#8239;' . $sec . '″&#8239;' . i18n( 'sdir-' . [
      'lat' => $decimal < 0 ? 'S' : 'N', 'lon' => $decimal < 0 ? 'W' : 'E'
    ][ $type ] );

  }

  function __DMS_coords ( float $lat, float $lon ) {
    return '<span>' . __DMS( $lat, 'lat' ) . '</span> <span>' . __DMS( $lon, 'lon' ) . '</span>';
  }

  function __cardinal ( float $hdg, bool $short = true ) {
  ...
  function __HDG ( float $p1_lat, float $p1_lon, float $p2_lat, float $p2_lon ) {
  ...
  function __HDG_bug ( float $hdg = 0, string $empty = '' ) {
  ...
  function __nearby ( float $dist, array $from, array $to ) {
  ...
  function __number ( float $number, int $digits = 0 ) {
    return number_format( $number, $digits, i18n_save( 'dp' ) ?? '.', i18n_save( 'tp' ) ?? ',' );
  }

  function __timediff ( int $minutes ) {

    if ( $minutes >= 1440 ) return floor( $minutes / 1440 ) . '&nbsp;' . i18n( 'timediff-days' );
    else if ( $minutes >= 60 ) return floor( $minutes / 60 ) . '&nbsp;' . i18n( 'timediff-hours' );
    else return $minutes . '&nbsp;' . i18n( 'timediff-minutes' );

  }

  function __mach ( float $spd, float $alt = 0 ) {

    if ( $alt <= 36152 ) $temp = 518.6 - 3.56 * ( $alt / 1000 );
    else if ( $alt <= 82345 ) $temp = 389.98;
    else if ( $alt <= 155348 ) $temp = 389.98 + 1.645 * ( $alt - 82345 ) / 1000;
    else if ( $alt <= 175346 ) $temp = 508.788;
    else $temp = 508.788 - 2.46888 * ( $alt - 175346 ) / 1000;

    return $spd * 1.151 / ( 15 / 22 * sqrt( 2405.2 * $temp ) );

  }

  function __morse ( string $input, bool $decode = false ) {
  ...
  function region_name ( string $type, string $region ) {
  ...
  function region_link ( string $type, string $region ) {
    return '<a href="' . SITE . 'airports/' . $type . '/' . $region . '">' .
      region_name( $type, $region ) . '</a>';
  }

  function tz_offset ( int $offset ) {
  ...
  function tz_link ( array $tz, bool $short = false, bool $offset = false ) {
  ...
  function breadcrumbs ( array $regions ) {
  ...
  function _breadcrumbs ( array $regions ) {
