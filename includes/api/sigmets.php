<?php
  ...
  foreach ( array_merge(
    ...
    $points = [];
    foreach ( $sigmet[ 'coords' ] ?? [] as $point ) $points[] = [ $point[ 'lon' ], $point[ 'lat' ] ];

    $polygon = json_encode( [ $points ], JSON_NUMERIC_CHECK );

    $affected += $DB->query( str_replace( '"NULL"', 'NULL', '
      INSERT INTO sigmet (
        _id, airport, fir, name, series, hazard, qualifier, severity, valid_from,
        valid_to, low_1, low_2, hi_1, hi_2, dir, spd, cng, raw, polygon
      ) VALUES (
        "' . $_id . '", "' . $airport . '", "' . $fir . '", "' . $name . '", "' . $series . '",
        "' . $hazard . '", "' . $qualifier . '", ' . $severity . ', "' . $valide_from . '",
        "' . $valide_to . '", ' . $low_1 . ', ' . $low_2 . ', ' . $hi_1 . ', ' . $hi_2 . ',
        "' . $dir . '", ' . $spd . ', "' . $cng . '", "' . $raw . '", "' . $polygon . '"
  ...
  api_exit( [ 'sigmets' => 'updated', 'affected' => $affected ] );
