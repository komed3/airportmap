<?php
  ...
  function wiki ( string $query, string $lang = 'en' ) {

    return json_decode( file_get_contents(
      'https://' . $lang . '.wikipedia.org/w/api.php?action=query&format=json&' . $query
    ), true )[ 'query' ];

  }

  function wiki_page ( array $res ) {
...
<html lang="en-US">
  ...
  <body>
    ...
    foreach ( [ 'en', 'de', 'fr' ] as $lang ) {
      ...
      if (
        ! empty( $check = wiki( 'prop=pageprops&ppprop=disambiguation&titles=' . $search, $lang ) ) &&
        ...
        // fetch images
          ...
          if ( ! empty( $images = wiki_page( wiki( 'prop=images&imlimit=max&redirects=1&&titles=' . $search, $lang ) )[ 'images' ] ) ) {
            ...
            foreach ( $images as $img ) {
              ...
              if (
                ! in_array( $img[ 'title' ], [ 'File:Aviacionavion.png' ] ) &&
          ...
          if( ! $has_images ) ?><p class="error">Could not found any images!</p><?php
