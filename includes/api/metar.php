<?php
  ...
  $sc_points = [ 'SKC' => 0, 'CLR' => 0, 'FEW' => 1, 'SKC' => 2, 'SCT' => 2, 'BKN' => 3, 'OVC' => 4, 'OVX' => 4 ];
  $delete = $insert = [];

  foreach ( explode( PHP_EOL, file_get_contents( 'https://aviationweather.gov/data/cache/metars.cache.csv' ) ) as $metar ) {

    $data = explode( ',', $metar );

    if ( count( $data ) != 44 || $data[ 1 ] == 'station_id' || strlen( $data[ 17 ] ) ) continue;
    ...
    if ( strlen( trim( $data[ 21 ] ) ) ) {
      ...
      foreach ( $data as $_i => $sc ) {
        ...
        if ( in_array( $_i, [ 22, 24, 26, 28 ] ) && ! empty( $sc ) ) {
          ...
          $sc_raw[] = array_key_exists( $key, $sc_points ) ? $sc_points[ $key ] : 0;

        }

      }

      $wx = empty( $sc_raw ) ? 'CAVOK' : [ 'CLR', 'FEW', 'SCT', 'BKN', 'OVC' ][ max( $sc_raw ) ];
