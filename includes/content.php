<?php
  ...
  function _base_url ( string $url = '' ) {
    echo base_url( $url );
  }

  function __redirect ( string $to ) {
  ...
  function check_tpl ( string $tpl ) {
    return is_readable( TEMPLATE . $tpl . '.php' );
  }

  function load_tpl_part ( string $tpl, string $fallback = '' ) {

    if ( check_tpl( $tpl ) ) include TEMPLATE . $tpl . '.php';
    else if ( check_tpl( $fallback ) ) include TEMPLATE . $fallback . '.php';
  ...
  function _site_end () {
    ...
    if ( ( $_COOKIE[ 'theme' ] ?? 'light' ) == 'dark' && ( ! defined( 'EMBED' ) || EMBED == false ) )
  ...
  function get_file ( string $file, string $size = '' ) {
    return RESOURCE . 'images/' . $size . $file;
  }

  function _get_file ( string $file, string $size = '' ) {
    echo get_file( $file, $size );
  }

  function site_nav ( array $links = [], string $classes = '', int $check_path = 0, $jump_to = null ) {
    ...
    <nav class="' . $classes . '">' .
      implode( '', array_map( function( $link ) use ( $equal, $jump_to ) {
        ...
        return $link ? '<a href="' .
          ...
          ( $jump_to ? '#' . $jump_to : '' ) .
        '" class="' .
          $link[ 'classes' ] . ' ' . ( ( $link['check'] ?? '' ) == $equal ? 'current' : '' ) .
        '" target="' .
  ...
  function _site_nav ( array $links = [], string $classes = '', int $check_path = 0, $jump_to = null ) {
    echo site_nav( $links, $classes, $check_path, $jump_to );
  }

  function pagination ( int $results, int $page = 1, string $baseurl = '', int $per_page = 24, $jump_to = '_pagination' ) {
    ...
    foreach ( array_filter( array_unique( [
      1, max( 1, $page - 2 ), max( 1, $page - 1 ), $page, min( $maxpage, $page + 1 ),
      min( $maxpage, $page + 2 ), $maxpage
    ] ) ) as $pageno ) {
    ...
    if ( count( $pagelinks ) <= 1 ) $pagelinks = [];
  ...
  function _pagination ( int $results, int $page = 1, string $baseurl = '', int $per_page = 24 ) {
    echo pagination( $results, $page, $baseurl, $per_page );
  }

  function back_to ( string $page, string $name, string $classes = '' ) {
  ...
  function _back_to ( string $page, string $name, string $classes = '' ) {
    echo back_to( $page, $name, $classes );
  }

  function pagelist ( string $page, array $results ) {

    $list = '';

    foreach ( $results as $row )  $list .= '<div><a href="' . base_url( $page . '/' . $row[ 'page' ] ) . '">
  ...
  function _pagelist ( string $page, array $results ) {
    echo pagelist( $page, $results );
  }

  function share_links ( $sites = [ 'twitter', 'facebook', 'telegram', 'tumblr', 'reddit' ] ) {
    ...
    foreach ( $sites as $site ) $links[] = '<a href="#" data-action="share" data-site="' . $site . '" data-url="' .
      $url . '" data-text="' . $text . '" class="social ' . substr( $site, 0, 2 ) . '" target="_blank" title="' .
  ...
  function _share_links ( $sites = [ 'twitter', 'facebook', 'telegram', 'tumblr', 'reddit' ] ) {
    echo share_links( $sites );
  }

  function add_resource ( string $resource, string $type, string $url ) {
    ...
    if (
      ! empty( $dir = [ 'css' => 'styles/', 'js' => 'scripts/' ][ strtolower( trim( $type ) ) ] ?? null ) &&
      ! array_key_exists( $res_id, (array) $__static_files )
    ) {
