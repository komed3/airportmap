<?php
  ...
  function temp_in ( float $temp, string $in = 'c', int $digits = 0 ) {
    return '<x>' . __number( $temp, $digits ) . '</x>&#8239;' . i18n( 'temp-' . $in );
  }

  function altim_in ( float $altim, string $in = 'inhg', int $digits = 2 ) {
    return '<x>' . __number( $altim, $digits ) . '</x>&#8239;' . i18n( 'altim-' . $in );
  }

  function wind_in ( float $wind, string $in = 'kt' ) {
    return '<x>' . __number( $wind ) . '</x>&#8239;' . i18n( 'wind-' . $in );
  }

  function precip_in ( float $precip, string $in = 'in', int $digits = 2 ) {
    return '<x>' . __number( $precip, $digits ) . '</x>&#8239;' . i18n( 'precip-' . $in );
  }

  function wind_info ( array $weather, bool $gust = false ) {

    return i18n(
      'wind-' . ( ( $weather[ 'wind_spd' ] ?? 0 ) == 0 ? 'calm' : 'to' ) .
  ...
  function vis_info ( array $weather, bool $labels = false ) {
  ...
  function wx ( array $weather ) {

    $text = [];

    if ( empty( $weather[ 'wx' ] ) || in_array( $weather[ 'wx' ], [ 'CAVOK', 'CLR', 'FEW', 'SCT', 'BKN', 'OVC' ] ) )
    ...
    foreach ( array_unique( explode( ' ', $weather[ 'wx' ] ) ) as $symbol ) {
      ...
      $parts = array_filter( array_map( function ( $p ) { return i18n_save( 'wx-' . $p ); }, $matches ) );

      if ( ! empty( array_intersect( $matches, [ 'TS', 'SH' ] ) ) ) array_pop( $parts );
  ...
  function wx_icon ( array $weather ) {
    ...
    foreach ( [
      ...
      if ( stripos( $wx, $probe ) !== false ) return $icon;

    }

  }

  function relhum ( array $weather ) {
    return 100 * (
  ...
  function sky_chart ( array $weather ) {
  ...
  function wind_rwy ( array $weather, array $airport ) {
    ...
    ' )->fetch_all( MYSQLI_ASSOC ) as $rwy ) {
      ...
      if ( $rwy[ 'l_hdg' ] != null ) $runways[ $rwy[ 'l_hdg' ] ][] = $rwy[ 'l_ident' ];
      if ( $rwy[ 'r_hdg' ] != null ) $runways[ $rwy[ 'r_hdg' ] ][] = $rwy[ 'r_ident' ];
  ...
  function remarks ( array $weather ) {

    $raw = $weather[ 'raw' ] . '  ' ?? '';
    $remarks = [];

    if ( preg_match( '/ PK WND ([0-9]{3})([0-9]{2})\/([0-9]{2})([0-9]{2}) /U', $raw, $matches ) ) $remarks[] = '<li>
      <span>' . i18n( 'remarks-peak-wind-label' ) . '</span>
      <div>' . i18n( 'remarks-peak-wind',
        wind_in( $matches[ 2 ] ), __cardinal( $matches[ 1 ] ),
        $matches[ 3 ], $matches[ 4 ]
      ) . '</div>
    </li>';

    if ( preg_match(
      '/ (FZ|SH|TS|BR|DS|DU|DZ|FC|FG|FU|GR|GS|HZ|IC|PE|PO|PY|RA|SA|SG|SN|SQ|SS|VA)(B|E)' .
    ...
    if ( preg_match_all( '/ (P|6|7)([0-9]{4}) /U', $raw, $matches ) ) {
      ...
      for ( $i = 0; $i < count( $matches[ 0 ] ); $i++ ) $remarks[] = '<li>
        <span>' . i18n( 'remarks-precip-label' ) . '</span>
        <div>' . i18n( 'remarks-precip',
          precip_in( $matches[ 2 ][ $i ] / 100, 'in' ),
          i18n( 'remarks-period-' . [ 'P' => '1', '6' => '6', '7' => '24' ][ $matches[ 1 ][ $i ] ] )
        ) . '</div>
      </li>';

    }

    if ( preg_match( '/ T(0|1)([0-9]{3})(0|1)([0-9]{3}) /U', $raw, $matches ) ) $remarks[] = '<li>
    ...
    if ( preg_match( '/ 4(0|1)([0-9]{3})(0|1)([0-9]{3}) /U', $raw, $matches ) ) $remarks[] = '<li>
      <span>' . i18n( 'remarks-temp-label' ) . '</span>
    ...
    if ( preg_match( '/ WSHFT ([0-9]{2})([0-9]{2}) (FROPA)? /U', $raw, $matches ) ) $remarks[] = '<li>
      <span>' . i18n( 'remarks-wind-shift-label' ) . '</span>
      <div>' . i18n( 'remarks-wind-shift' . ( $matches[ 3 ] ? '-fropa' : '' ), $matches[ 1 ], $matches[ 2 ] ) . '</div>
    </li>';

    if ( preg_match( '/ SLP([0-9]{3}) /U', $raw, $matches ) ) $remarks[] = '<li>
      <span>' . i18n( 'remarks-sealevel-label' ) . '</span>
      <div><b>' . altim_in( $matches[ 1 ] * 10, 'hPa', 0 ) . '</b></div>
    </li>';

    if ( preg_match( '/ PRES(F|R)R /U', $raw, $matches ) ) $remarks[] = '<li>
      <span>' . i18n( 'remarks-pressure-label' ) . '</span>
      <div>' . i18n( 'remarks-pressure-' . $matches[ 1 ] ) . '</div>
    </li>';

    if ( preg_match( '/ (AUTO|COR) /U', $raw, $matches ) ) $remarks[] = '<li>
      <span>' . i18n( 'remarks-report-label' ) . '</span>
      <div>' . i18n( 'remarks-report-' . $matches[ 1 ] ) . '</div>
    </li>';

    if ( preg_match( '/ AO(1|2) /U', $raw, $matches ) ) $remarks[] = '<li>
    ...
    if ( strpos( $raw, '$' ) != false ) $remarks[] = '<li>
  ...
  function stations_at ( float $lat, float $lon, int $limit = 10, int $max_deg = 15, int $max_age = 1, string $test_time = 'now' ) {
  ...
  function sigmet_hazard ( array $sigmet ) {

    $h = i18n_save( 'hazard-' . $sigmet[ 'hazard' ] );
    $q = i18n_save( 'qualifier-' . $sigmet[ 'qualifier' ] );

    return ucfirst( empty( $q ) ? $h : i18n( 'sigmet-hazard', $h, $q ) );

  }

  function sigmet_valid ( array $sigmet ) {

    $valid_from = strtotime( $sigmet[ 'valid_from' ] );
    $valid_to = strtotime( $sigmet[ 'valid_to' ] );

    return i18n( date( 'd', $valid_from ) != date( 'd', $valid_to ) ? 'valid-until' : 'valid-from-to',
      date( 'm/d', $valid_from ), date( 'H:i', $valid_from ),
      date( 'm/d', $valid_to ), date( 'H:i', $valid_to )
    );

  }

  function sigmet_move ( array $sigmet ) {
  ...
  function sigmet_cng ( array $sigmet ) {
    return ucfirst( i18n( 'change-' . ( $sigmet[ 'cng' ] ?? 'NC' ) ) );
  }

  function sigmet_fl ( array $sigmet ) {

    $fl_msgkey = 'fl';

    if ( ! empty( $fl_base = $sigmet[ 'low_1' ] ?? ( $sigmet[ 'low_2' ] ?? null ) ) ) $fl_msgkey .= '-from';
    if ( ! empty( $fl_top = $sigmet[ 'hi_1' ] ?? ( $sigmet[ 'hi_2' ] ?? null ) ) ) $fl_msgkey .= '-to';
  ...
  function sigmet_info ( array $sigmet, bool $location = true ) {
  ...
  function station_list ( array $stations, int $page = 1, string $baseurl = '' ) {
  ...
  function _station_list ( array $stations, int $page = 1, string $baseurl = '' ) {
