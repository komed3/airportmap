...
const number_format = ( number, options = {} ) => new Intl.NumberFormat(
  $.cookie( 'locale' ) || 'en', options
).format( number );

const freq_format = ( frequency ) => {
  frequency = parseInt( frequency );

  return frequency > 1000 ? number_format( frequency / 1000, {
    minimumFractionDigits: 1, maximumFractionDigits: 3
  } ) + '&#8239;MHz' : number_format( frequency ) + '&#8239;kHz';
};
...
( function ( $ ) {
  $( document ).ready( function () {
    ...
    if ( 'serviceWorker' in navigator ) navigator.serviceWorker.register( baseurl + '/service-worker.js', { scope: '/' } );
  ...
  $( document ).scroll( function () {
    if( $( window ).scrollTop() > 200 ) $( 'body' ).addClass( 'scroll-to-top' );
    else $( 'body' ).removeClass( 'scroll-to-top' );
  } );
} )( jQuery );
