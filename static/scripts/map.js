const
  maps_config = {}, maps = {}, maps_type = {}, maps_layer = {}, maps_timeout = {},
  maps_loaded = {}, maps_mypos_marker = {}, map_sigmet_colors = {
    CLD: '#595b64', CONV: '#aa66ff', DS: '#eebb55', FC: '#aa66ff',
    GR: '#ff2200', ICE: '#4488dd', MTW: '#44aa44', SS: '#eebb55',
    TC: '#dd66ee', TDO: '#ff2200', TS: '#ff2200', TSGR: '#ff2200',
    TURB: '#ee8844', VA: '#595b64', WTSPT: '#4488dd'
  };
...
( function ( $ ) {
  const checkLimit = () => {
    if ( 'userAgentData' in navigator && 'mobile' in navigator.userAgentData ) maps_limit = 50;
    else if ( ( navigator.hardwareConcurrency || 4 ) < 2 || ( navigator.deviceMemory || 2 ) < 1 ) maps_limit = 80;
    else maps_limit = 140;
  };

  const map_check_size = ( uuid ) => {
    const { x, y } = maps[ uuid ].getSize();

    if ( x < 600 || y < 400 ) $( '[uuid="' + uuid + '"]' ).addClass( 'mini-map micro-map' );
    if ( x < 900 || y < 600 ) $( '[uuid="' + uuid + '"]' ).addClass( 'mini-map' );
    else $( '[uuid="' + uuid + '"]' ).removeClass( 'mini-map micro-map' );
  };
  ...
  const map_halo = ( uuid, position = false ) => {
    if ( maps_layer[ uuid ].halo ) maps[ uuid ].removeLayer( maps_layer[ uuid ].halo );

    if ( position !== false ) maps_layer[ uuid ].halo = L.marker( position, {
      icon: L.divIcon( { iconSize: [ 72, 72 ], iconAnchor: [ 36, 36 ], className: 'halo', html: '<halo></halo>' } )
    } ).addTo( maps[ uuid ] );
  };

  const map_check_zoom = ( uuid ) => {
    const zoom = maps[ uuid ].getZoom();
  ...
  const map_remove_layer = ( uuid, layer, clear = false ) => {
  ...
  const map_load_marker = ( uuid ) => {
    if ( 'marker' in maps_timeout[ uuid ] ) clearTimeout( maps_timeout[ uuid ].marker );

    const map = maps[ uuid ], bounds = map.getBounds(), layer = maps_layer[ uuid ].marker;

    let data = {
      token: get_token(), limit: maps_limit, bounds: {
        lat: [ bounds.getNorth(), bounds.getSouth() ],
        lon: [ bounds.getEast(), bounds.getWest() ]
      }, navaids: +!! ( map.getZoom() >= 10 && (
        maps_config[ uuid ].navaids || ( $.cookie( 'apm_navaids' ) || 0 )
      ) == 1 ), waypoints: +!! ( map.getZoom() >= 10 && (
        maps_config[ uuid ].waypoints || ( $.cookie( 'apm_waypoints' ) || 0 )
      ) == 1 )
    };

    if ( 'query' in maps_config[ uuid ] ) data = { ...data, ...maps_config[ uuid ].query };

    $.ajax( { url: apiurl + maps_type[ uuid ] + '_layer.php', type: 'post', data: data, success: raw => {
        ...
        if ( 'navaids' in res.response )
          Object.values( res.response.navaids ).forEach( navaid => {
            layer.addLayer( L.marker(
              L.latLng( parseFloat( navaid.lat ), parseFloat( navaid.lon ) ),
              { icon: L.divIcon( {
                iconSize: [ 24, 24 ], iconAnchor: [ 12, 12 ],
                className: 'navaid-' + navaid.type, html: '<navicon></navicon>'
              } ) }
            ).bindTooltip(
              '<div class="IDENT">' + navaid.ident + '</div>' +
              '<div class="freq">' + navaid.type + ' ' + freq_format( navaid.frequency ) + '</div>',
              { className: 'tooltip-navaid', direction: 'center', opacity: 1 }
            ).on( 'click', e => map_navaid_info( e, uuid, navaid ) ) );
          } );

        if ( 'waypoints' in res.response )
          Object.values( res.response.waypoints ).forEach( waypoint => {
            layer.addLayer( L.marker(
              L.latLng( parseFloat( waypoint.lat ), parseFloat( waypoint.lon ) ),
              { interactive: false, icon: L.divIcon( {
                iconSize: [ 20, 20 ], iconAnchor: [ 10, 10 ],
                className: 'waypoint', html: '<wpicon></wpicon>'
              } ) }
            ).bindTooltip(
              '<div class="IDENT">' + waypoint.ident + '</div>',
              { className: 'tooltip-waypoint', direction: 'center', permanent: true, opacity: 1 }
            ) );
          } );
        ...
        if ( 'airports' in res.response )
          Object.values( res.response.airports ).forEach( airport => {
            layer.addLayer( L.marker(
              L.latLng( parseFloat( airport.lat ), parseFloat( airport.lon ) ),
              { icon: L.divIcon( {
                iconSize: [ 20, 20 ], iconAnchor: [ 10, 10 ], html: '<mapicon></mapicon>',
              ...
              { className: 'tooltip-airport', direction: 'center', opacity: 1 }
            ).on( 'click', e => map_airport_info( e, uuid, airport ) ) );
          } );

        if ( 'stations' in res.response )
          Object.values( res.response.stations ).forEach( ( station ) => {
            layer.addLayer( L.marker(
              L.latLng( parseFloat( station.lat ), parseFloat( station.lon ) ),
              { icon: L.divIcon( {
                iconSize: [ 28, 28 ], iconAnchor: [ 14, 14 ], className: 'cat-' + station.cat,
              ...
              { className: 'tooltip-airport', direction: 'center', opacity: 1 }
            ).on( 'click', e => map_airport_info( e, uuid, station ) ) );
          } );

        if ( 'traffic' in res.response )
          Object.values( res.response.traffic ).forEach( tfc => {
            layer.addLayer( L.marker(
              L.latLng( parseFloat( tfc.lat ), parseFloat( tfc.lon ) ),
              { icon: L.divIcon( {
                iconSize: [ 24, 24 ], iconAnchor: [ 12, 12 ],
              ...
              '</div>',
              { className: 'tooltip-traffic', direction: 'center', opacity: 1 }
            ).on( 'click', e => map_traffic_info( e, uuid, tfc ) ) );
          } );

        if ( maps_type[ uuid ] == 'weather' ) maps_timeout[ uuid ].marker =
          setTimeout( () => map_load_marker( uuid ), 300000 );
      }
    } );
  };
  ...
  var map_sigmets_update = ( uuid ) => {
    try {
      if ( ! uuid || ! maps_layer[ uuid ] || ! ( 'sigmet' in maps_layer[ uuid ] ) ) return;

      let layer = maps_layer[ uuid ].sigmet;
      if( ! layer ) return;

      $.ajax( { url: apiurl + 'sigmet_layer.php', type: 'post', data: { token: get_token() }, success: raw => {
        try {
          ...
          try {
            Object.values( sigmets ).forEach( function ( sigmet ) {
              if ( ! sigmet || typeof sigmet !== 'object' ) return;
              ...
              try {
                ...
                if ( typeof polygonData === 'string' ) {
                  try { polygonArray = JSON.parse( polygonData ) }
                  catch ( parseErr ) {
                ...
                if ( ! Array.isArray( polygonArray ) || polygonArray === null || polygonArray.length === 0 ) return;

                polygonArray.forEach( function ( polygon ) {
                  try {
                    // Strict polygon validation
                    if ( ! polygon || ! Array.isArray( polygon ) || polygon.length < 2 ) return;
                    ...
                    if ( reversedPolygon.length < 2 ) return;

                    try {
                      let poly = L.polygon( reversedPolygon, {
                        color: hazard_color, weight: 1, fillOpacity: 0.15, dashArray: '4 4'
                      } );
                      ...
                      if ( poly && sigmet.hazard && typeof sigmet.hazard === 'string' ) {
                        try {
                          poly.bindTooltip(
                            ...
                            className: 'tooltip-sigmet', direction: 'center', opacity: 1, permanent: true
                          } );
                        } catch ( tooltipErr ) {
                      ...
                      if ( poly ) {
                        try {
                          poly.on( 'click', e => {
                            try { map_sigmet_info( poly, e, uuid, sigmet ) }
                            catch ( clickErr ) { console.warn( 'Error in sigmet click handler:', clickErr ) }
                          } );
                        } catch ( eventErr ) {
                      ...
                      if ( poly && layer ) {
                        try { layer.addLayer( poly ) }
                        catch ( addErr ) { console.warn( 'Error adding layer:', addErr ) }
                      }
                    } catch ( polyErr ) {
          ...
          if ( maps_timeout[ uuid ] && typeof maps_timeout[ uuid ] === 'object' ) {
            try {
              ...
              maps_timeout[ uuid ].sigmet = setTimeout( () => {
                try { map_sigmets_update( uuid ) }
                catch ( updateErr ) { console.error( 'Error in scheduled SIGMET update:', updateErr ) }
              }, 60000 );
            } catch ( timeoutErr ) {
        ...
        if ( maps_timeout[ uuid ] && typeof maps_timeout[ uuid ] === 'object' ) {
          try {
            ...
            maps_timeout[ uuid ].sigmet = setTimeout( () => {
              try { map_sigmets_update( uuid ) }
              catch ( retryErr ) { console.error( 'Error in retry SIGMET update:', retryErr ) }
            }, 60000 );
          } catch ( timeoutErr ) {
